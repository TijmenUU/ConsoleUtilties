...
{
	ExtIO::Color originalColor(0U);
	std::cout >> originalColor;

	for (std::uint16_t i = 0x00; i <= 0x0F; ++i)
	{
		std::uint8_t bgColor = static_cast<std::uint8_t>(ExtIO::Color::Background::BLACK);
		std::cout << std::setw(4) << ExtIO::Color(bgColor | i) << i << ' ';
	}
	...
	{
		std::uint8_t fontColor = static_cast<std::uint8_t>(ExtIO::Color::Font::WHITE);
		std::cout << std::setw(4) << ExtIO::Color(fontColor | i) << ' ';
	}
...
{
	ExtIO::Window::Rectangle rect;
	std::cout >> rect;
	ExtIO::Coordinate size = rect.bottomRight - rect.topLeft;
...
{
	ExtIO::Coordinate originalCoord;
	std::cout >> originalCoord;
	ExtIO::Color originalColor(0);
	std::cout >> originalColor;

	std::cout << std::setprecision(3) << std::fixed << "Pasting Lorum ipsum in ";
	ExtIO::Coordinate timeCoord;
	std::cout >> timeCoord;
	...
	std::cout << originalCoord;
	std::cout << ExtIO::Color(ExtIO::Color::Font::GREEN);
